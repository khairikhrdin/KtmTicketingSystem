...
namespace KTMTicketingv2.Controllers
{
    public class CRUDUsersController : Controller
    {
        ...
        public async Task<ActionResult> Create([Bind(Include = "UserID,FirstName,LastName,EmailID,Password,IsEmailVerified,ActivationCode,ICNumber")] User user)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "UserID,FirstName,LastName,EmailID,Password,IsEmailVerified,ActivationCode,ICNumber")] User user)
        {
            if (ModelState.IsValid)
            {
